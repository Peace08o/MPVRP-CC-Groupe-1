# mpvrpcc_solver_large.py
# MPVRP-CC – Chargement depuis le dossier "large"
...
def read_instance(path):
    ...
    # Matrice de distance des garages (ignorée)
    idx += G
...
def write_solution(path, sol, depots, start_time):
    ...
    with open(path, "w") as f:
        ...
        # Métriques finales (6 lignes obligatoires)
        f.write(str(len(sol)) + "\n")          # nb véhicules utilisés
        f.write(str(total_changes) + "\n")     # nb changements produit
        f.write("0.0\n")                        # coût de changement
...
# -------------------------------------------------
# Résolution de toutes les instances du dossier large
# -------------------------------------------------

def solve_all():
    INSTANCE_DIR = "large"       # 👈 DOSSIER DEMANDÉ
